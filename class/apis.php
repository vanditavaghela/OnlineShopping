...
require_once  'db.php'; 
class MyShoppie extends DB
{
	public function __construct() {
		// parent::__construct();
		$this->userid    = (!empty($_SESSION["userid"])) ? $_SESSION["userid"] : null;
		$this->username  = (!empty($_SESSION["username"])) ? $_SESSION["username"] : "Guest";
		$this->table     = " test.tbl_users_cart ";
		$this->usertable = " test.tbl_userdetails ";
		$this->isaccess  = 0;
		$this->userauth();
	}
	...
	public function callCurl($apiurl) {
        ...
        $output = curl_exec($ch);
        $result = json_decode($output, 1);print_r($res);exit;
        curl_close($ch);

		// return $result;
	}

	public function productData() { /*fake data from https://fakestoreapi.com*/
		$json_data = '
	...
	public function addtocart($id) {
		...
		if($this->isaccess == 1) {
			...
			$conn 	= new DB();
			$query  = "insert into ".$this->table." (userid, productid, update_time)
						VALUES ('".$this->userid."','".$id."','".date('Y-m-d H:i:s')."')";
			// echo  $query;exit;
			$res 	= $conn->execQry($query);
			if($res) {
				$rowcount = $this->getInCartProducts();
			}
		} 
		return array("data"=>$rowcount);
	}

	public function getInCartProducts() {
		if($this->isaccess == 1) {

			$conn 	  = new DB();
			$query	  = "SELECT COUNT( DISTINCT productid ) as cnt FROM ".$this->table." WHERE userid = '".$this->userid."' AND active_cart =1";
			$res 	  = $conn->execQry($query);
			$result	  = $conn->fetchAll($res);
	...
	public function getUserProducts() {
		...
		if($this->isaccess == 1) {
			...
			$conn 	  = new DB();
			$query	  = "select DISTINCT productid FROM ".$this->table." WHERE userid = '".$this->userid."' AND active_cart =1";
			$res 	  = $conn->execQry($query);
			// while($row	  = $conn->fetchAll($res)) {
	...
	public function placeorder() {
		...
		if($this->isaccess == 1) {
			...
			$query  = "update ".$this->table." 
						SET active_cart=0 where userid='".$this->userid."' ";
			$res 	= $conn->execQry($query);
			if($res) {
				$result = true; 
			}
	...
	public function userauth() { // user authentication to check if user is logged in or not
		$isaccess = 0; 
	...
	public function login($params) {
		...
		$result= array(); $result["status"] = false;
		$conn  = new DB();
		$query = "select userid,username,logged_in from ".$this->usertable. " 
					WHERE active=1 AND username='".$user."' AND password='".$pwd."' limit 1";
		$res   = $conn->execQry($query);
		while ($row = mysqli_fetch_assoc($res)) {
			$this->userid    = $row["userid"];
			$this->username  = $row["username"];

			$result["userid"] 	= $_SESSION["userid"] 	= $this->userid;
			$result["username"] = $_SESSION["username"] = $this->username;
